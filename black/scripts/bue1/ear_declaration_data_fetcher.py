...
def enter_company_by_weee(page: Page, portal_frame, weee_number: str, log_prefix: str = "") -> bool:
    ...
    for _ in range(5):
        refreshed_frame = get_portal_frame(page)
        if is_ist_input_page_ready(refreshed_frame) or is_locator_visible(
            get_ist_input_link(refreshed_frame),
            timeout_ms=800,
        ):
            return True
...
def attempt_login_and_fetch(
    ...
    try:
        ...
        if reuse_session:
            ...
            try:
                login_form_still_visible = account_input.is_visible(timeout=3000)
            except Exception:
                login_form_still_visible = False
        ...
        if login_form_still_visible:
            ...
            if ist_input_opened:
                ...
                result["status"] = "weight_found" if matched_weight else "matching_entry_not_found"
            else:
                result["route"] = "company_selection_required"
                result["status"] = "company_selection_required"
