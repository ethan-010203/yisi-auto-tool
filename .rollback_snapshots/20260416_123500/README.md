...
```bash
cd black
pip install fastapi uvicorn pydantic pymupdf openai python-multipart
```
...
- 前端默认端口：`5173`
- 后端默认端口：`8000`
- 如果正式部署在 Windows，前端页面中应保存“服务端电脑可访问”的 Windows 路径，例如：
...
- 如果当前在 Mac 上开发测试，共享目录需要先挂载到本机，再填写实际挂载路径，例如 `/Volumes/厦门部门/BUE2`
