...
async def upload_invoice_folder(
    ...
    session_root = _build_upload_session_root("CONSULT", "invoice_recognizer", "folder")
...
async def upload_invoice_excel(file: UploadFile = File(...)):
    ...
    session_root = _build_upload_session_root("CONSULT", "invoice_recognizer", "excel")
...
def test_network_path(request: NetworkPathTestRequest):
    ...
    try:
        ...
        if not (is_unc_path or is_posix_path or is_windows_drive):
            return {
                "success": False,
                "error": "路径格式错误：请填写服务端可访问的绝对路径。Windows 部署示例：\\\\\\\\服务器\\\\共享文件夹\\\\BUE2 或 D:\\\\工作目录；macOS 开发示例：/Volumes/共享目录/BUE2。",
            }

        if is_unc_path and os.name != "nt":
            return {
                "success": False,
                "error": "当前服务不是运行在 Windows 上。开发测试时请先把共享目录挂载到本机，再填写挂载后的实际路径，例如 /Volumes/厦门部门/BUE2。",
            }

        # 将路径转换为 Path 对象
        test_path = Path(expanded_path)
