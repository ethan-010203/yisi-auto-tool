...
async function handleInvoiceExcelSelected(event) {
  ...
  try {
    ...
    pushToast({
      type: 'success',
      title: 'Excel 已上传',
      message: '已将当前电脑选择的 Excel 复制到服务端运行区。',
    })
  } catch (error) {
    pushToast({
      type: 'error',
...
async function runDepartmentScript(tool) {
  ...
  try {
    ...
    if (tool.id === 'invoice_recognizer') {
      const configSource = currentConfigTool.value.department === departmentCode && currentConfigTool.value.toolId === tool.id
        ? configData.value
        : (await getToolConfig(departmentCode, tool.id))?.config || {}
      ...
      <UiCard class="department-card">
        ...
        <div class="department-config-section">
          ...
          <div class="test-result">
            正式部署在 Windows 时，请填写服务端电脑可访问的 Windows 路径，例如 `\\服务器\共享文件夹\BUE2` 或 `D:\工作目录\BUE2`。如果当前仍在 Mac 开发测试，可填写挂载后的 `/Volumes/...` 路径。
          </div>
    ...
    >
      ...
      <div v-if="currentConfigTool.toolId !== 'citeo_email_extractor'" class="config-form">
        <div class="form-field">
          ...
          >
          <small class="field-hint">会在当前打开网页的这台电脑上弹窗选择，并把所选文件夹上传到部署电脑供脚本运行。推荐使用 Windows Edge 或 Chrome。</small>
        </div>
        ...
        <div class="form-field">
          ...
          >
          <small class="field-hint">选择后会先上传到部署电脑，再由脚本读取上传副本，不会去访问同事电脑的本地路径。</small>
        </div>
      </div>
