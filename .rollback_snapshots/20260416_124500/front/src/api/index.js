...
export function runDepartmentTool(department, tool, configOverride = null, signal) {
  return requestJson(`${API_BASE_URL}/departments/${department}/tools/${tool}/run`, {
    method: 'POST',
    body: JSON.stringify(configOverride ? { configOverride } : {}),
    signal,
  })
}
...
export function uploadInvoiceFolder(files, signal) {
  const formData = new FormData()
  let selectionName = ''

  Array.from(files || []).forEach((file) => {
    const relativePath = file.webkitRelativePath || file.name
    if (!selectionName && relativePath.includes('/')) {
      selectionName = relativePath.split('/')[0]
    }
  ...
  return requestJson(`${API_BASE_URL}/uploads/consult/invoice-recognizer/folder`, {
    method: 'POST',
    body: formData,
    signal,
  })
...
export function uploadInvoiceExcel(file, signal) {
  const formData = new FormData()
  formData.append('file', file, file.name)

  return requestJson(`${API_BASE_URL}/uploads/consult/invoice-recognizer/excel`, {
    method: 'POST',
    body: formData,
    signal,
  })
