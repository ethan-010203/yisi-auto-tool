...
function cleanup() {
  ...
  if (ctnDom.value) {
    ...
    if (gl?.canvas && ctnDom.value.contains(gl.canvas)) {
...
watch(() => [
  ...
  props.glowIntensity,
  props.saturation,
  props.mouseRepulsion,
  props.twinkleIntensity,
  props.rotationSpeed,
  props.repulsionStrength,
  props.autoCenterRepulsion,
  props.transparent
], () => {
  cleanup()
  initGalaxy()
...
<template>
  <div ref="ctnDom" class="galaxy-container"></div>
</template>
